...
class TasksFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = TaskAdapter(onCardClick = { t -> TaskDetailActivity.start(requireContext(), t.id) }, onStatusChange = { t, s -> vm.updateStatus(t.id, s) })
        binding.rvTasks.adapter = adapter
        vm.filteredTasks.observe(viewLifecycleOwner) { tasks ->
            adapter.submitList(tasks)
            binding.tvTaskCount.text = "${tasks.size} task${if (tasks.size != 1) "s" else ""}"
    ...
    private fun showSortMenu() {
        val options = arrayOf("Newest first", "Oldest first", "By priority", "By due date")
        val modes = arrayOf("new", "old", "prio", "due")
        androidx.appcompat.app.AlertDialog.Builder(requireContext()).setTitle("Sort tasks").setItems(options) { _, i -> vm.sortMode.value = modes[i] }.show()
    }
    private fun showCategoryFilter() {
        val cats = vm.settings.categories
        val checked = cats.map { vm.filterTypes.value?.contains(it) == true }.toBooleanArray()
    ...
    private fun showPriorityFilter() {
        ...
        val checked = prios.map { vm.filterPrios.value?.contains(it) == true }.toBooleanArray()
