...
class GalleryFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        vm.allTasks.observe(viewLifecycleOwner) { tasks ->
            ...
            tasks.forEach { t -> t.photos.forEach { src -> photos.add(PhotoItem(src, t.id, t.title)) } }
            adapter.items = photos; adapter.notifyDataSetChanged()
    ...
    private fun showPhotoViewer(item: PhotoItem) {
        ...
        iv.setOnClickListener { dialog.dismiss() }; dialog.show()
    }
    inner class PhotoAdapter(val onClick: (PhotoItem) -> Unit) : RecyclerView.Adapter<PhotoAdapter.VH>() {
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
            val b = ItemPhotoBinding.inflate(LayoutInflater.from(parent.context), parent, false); return VH(b)
        }
        override fun onBindViewHolder(holder: VH, position: Int) {
            val item = items[position]
            Glide.with(holder.itemView.context).load(item.src).centerCrop().into(holder.binding.ivPhoto)
